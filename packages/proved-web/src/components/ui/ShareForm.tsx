...
const shareUrl = ({ proofId }: { proofId: string }) => {
  ...
  return host + `/approve/${proofId}`;
};
export const ShareForm = ({ proofId, orgName, orgId }: ShareFormProps) => {
  const router = useRouter();
  const [copied, setCopied] = useState(false);
  const url = shareUrl({ proofId: proofId as string });
  const message = `Hey, can you prove my ${orgName} role on Proved? Here is the link 👉 ${url}`;
  ...
  return (
    <div className="w-full p-1 bg-gradient-to-r from-[#F8BCFF] via-[#9A96FF] to-[#A7DCFF] rounded-lg">
      <div className="px-4 py-4 sm:px-6 bg-gray-900  rounded-lg ">
        <p className="my-2 flex items-center text-center text-white ">
          Share the link and get proved! Only members in your DAO can prove this
          role.
        </p>
        <button
          onClick={handleCopy}
